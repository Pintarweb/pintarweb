...
- [x] Bot demo widget placeholder: `<!-- {{BOT_DEMO_WIDGET}} -->` in `landing/index.html` at line 414
- [x] Bot demo component: `components/bot-demo-widget/bot-demo-widget.html` (standalone HTML+CSS+JS)
- [x] Deploy script injection: `deploy-main.sh` injects bot-demo-widget before deploy — LIVE
...
- [x] Landing page overhaul (2026-07-06): RM297 entry pricing, 10 FAQ, How It Works 4-step, +60174456243 all CTAs, meta updated — LIVE at pintarweb.com
...
### 3.1.5 GMB / Google Business Profile Integration (2026-07-09)
- [x] GMB fields added to D1 schema (`packages/scraper/schema.sql`)
- [x] GMB detail scraper created (`packages/scraper/src/scrapers/gmbDetail.ts`)
- [x] ExpectedLead interface updated with GMB fields
- [x] upsertLead.ts updated to store GMB data
- [x] Main scraper integrated to call GMB scraper after Maps scrape
- [x] Audit template updated with G section (`templates/audit-template.html`)
- [x] generate-audit.sh updated to accept `--gmb` data and render G section
- [x] GMB Setup SOP documented (`docs/sop/gmb-setup.md`)
- [ ] Test GMB scraper end-to-end
- [ ] Test audit generation with GMB data

**GMB Status in Audit:** 4 states — TIADA GMB, BELUM SAH, TIDAK LENGKAP, TERVERIFIKASI  
**GMB Package:** Included in RM149/month (creation + optimization)  
